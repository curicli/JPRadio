...
手机上是一台收音机（卡片轮播选台，番組表走顶栏日历钮弹的抽屉）。屏幕宽了就把本来空着的
两侧填成有用的东西，而不是把收音机拉长：

| 宽度 | 布局 |
| --- | --- |
| < 980px | 单栏。卡片轮播选台，番組表 / 录音库都是抽屉 |
| ≥ 980px | 两栏：**电台列表**（左 340px）+ 播放器。鼠标用户不该被逼着横向滑卡片 |
| ≥ 1200px | 三栏：电台列表 + 播放器 + **常驻番組表**（右 380px）。番組表常驻了，顶栏那颗日历钮就隐藏（点开的抽屉和右栏内容一样） |

宽屏下**整窗高度是锁死的**（`#app` 撑满 `100dvh`），三栏一样高、各自在栏内滚动 ——
不锁的话 `1fr` 会退化成按内容取高，116 行的电台列表能把整页撑到六千多像素，
侧栏还会比播放器短一截。

常驻的番組表有**自己的日期游标**（`state.guideDay`），和抽屉的 `state.day` 分开：
抽屉是临时看一眼，常驻面板是长期挂着，共用一个游标会互相把日期顶掉。换台时它自动回到当天。

番組表每行右边的操作（预约 / 下载存档 / タイムフリー）**只用图标 + `aria-label`**，
不用文字标签 —— 「Schedule recording」这种长标签会把节目标题挤成窄窄一列、层层折行，
还跟「ON AIR」角标叠在一起。
